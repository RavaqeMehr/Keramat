...
const FamilyMemberRelationForm = () => {
	...
	const submit = () => {
		...
		if (id == 0) {
			axios
				.post('ValiNematan/AddFamilyMemberRelation', dto)
				.then((response) => response.data)
				.then(async (data) => {
					dispatch(await ReduxActions.logicActions.updateFamilyMemberRelations());
					navigate(`./../${data.data}`, { replace: true, relative: true });
				})
				.catch(console.error);
		} else {
			axios
				.put('ValiNematan/EditFamilyMemberRelation', { id, ...dto })
				.then((response) => response.data)
				.then(async (data) => {
					dispatch(await ReduxActions.logicActions.updateFamilyMemberRelations());
	...
	const GetPage = (page) => {
		...
		axios
			.get('ValiNematan/FamilyMemberRelationUsesList', { params: { id, page } })
			.then((x) => x.data.data)
			.then((x) => {
	...
	const remove = () => {
		axios
			.delete('ValiNematan/RemoveFamilyMemberRelation', { params: { id } })
			.then((x) => x.data.data)
			.then(async (x) => {
				if (x) {
					dispatch(await ReduxActions.logicActions.updateFamilyMemberRelations());
...
export default FamilyMemberRelationForm;
