...
const FamilyMemberSpecialDiseaseSubjectForm = () => {
	...
	const submit = () => {
		...
		if (id == 0) {
			axios
				.post('ValiNematan/AddFamilyMemberSpecialDiseaseSubject', dto)
				.then((response) => response.data)
				.then(async (data) => {
					dispatch(await ReduxActions.logicActions.updateFamilyMemberSpecialDiseaseSubjects());
					navigate(`./../${data.data}`, { replace: true, relative: true });
				})
				.catch(console.error);
		} else {
			axios
				.put('ValiNematan/EditFamilyMemberSpecialDiseaseSubject', { id, ...dto })
				.then((response) => response.data)
				.then(async (data) => {
					dispatch(await ReduxActions.logicActions.updateFamilyMemberSpecialDiseaseSubjects());
	...
	const GetPage = (page) => {
		...
		axios
			.get('ValiNematan/FamilyMemberSpecialDiseaseSubjectUsesList', { params: { id, page } })
			.then((x) => x.data.data)
			.then((x) => {
	...
	const remove = () => {
		axios
			.delete('ValiNematan/RemoveFamilyMemberSpecialDiseaseSubject', { params: { id } })
			.then((x) => x.data.data)
			.then(async (x) => {
				if (x) {
					dispatch(await ReduxActions.logicActions.updateFamilyMemberSpecialDiseaseSubjects());
	...
	return (
		<>
			<MyAccordion headers={['فرم', 'لیست وابسطه', 'حذف']}>
				<MyForm
					title={`${id == 0 ? 'افزودن' : 'ویرایش'} بیماری خاص`}
					onSubmit={submit}
